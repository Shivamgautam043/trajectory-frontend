...
import { Center, RingProgress, ThemeIcon, rem } from "@mantine/core";
    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 mb-6">
      ...
      <div className="flex items-center gap-6">
        <RingProgress
          ...
          thickness={8}
          sections={[
            { value: progress, color: progress >= 100 ? "green" : "blue" },
          ]}
          label={
            <Center>
              {progress >= 100 ? (
        ...
        <div className="flex flex-col">
          <p className="text-sm text-zinc-600 dark:text-zinc-300">
            {progress >= 100 ? "Goal crushed! 🚀" : `${remaining} more to go.`}
          </p>
          <p className="text-xs text-zinc-400 mt-1">Consistency is key.</p>
        </div>
      </div>
    </div>
  );
