...
} from "@/lib/backend/interviewRound";
import {
  Application,
...
export function ApplicationDetailsView({
  ...
  const [activeRound, setActiveRound] = useState<InterviewRound | null>(null);
  ...
  async function handleAddRound(formData: FormData) {
    ...
    const payload = {
      ...
      interviewer_name:
        formData.get("interviewer_name")?.toString() || undefined,
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <button
                      ...
                      className="text-left text-sm font-medium text-zinc-900 hover:font-bold! dark:text-zinc-200 transition-all duration-300 cursor-pointer"
                    >
                      {round.round_type.replace("_", " ")} (Round{" "}
                      {round.round_number})
                    </button>
        ...
        {activeRound && (
          <form
            onSubmit={async (e) => {
              ...
              const result = await updateInterviewRound(activeRound);
          ...
          >
            <Select
              label="Result"
              value={activeRound.result}
              onChange={(v) =>
                setActiveRound({ ...activeRound, result: v as any })
              }
              data={["PASSED", "FAILED", "PENDING", "SKIPPED"]}
            />

            <TextInput
              label="Interviewer Name"
              value={activeRound.interviewer_name ?? ""}
              onChange={(e) =>
                setActiveRound({
                  ...activeRound,
            ...
            <TextInput
              label="Meeting Link"
              value={activeRound.meeting_link ?? ""}
              onChange={(e) =>
                setActiveRound({ ...activeRound, meeting_link: e.target.value })
              }
            />
            ...
            <Textarea
              ...
              minRows={6}
              autosize
              value={activeRound.personal_notes ?? ""}
              onChange={(e) =>
                setActiveRound({
                  ...activeRound,
